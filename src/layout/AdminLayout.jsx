...
import { useAdmin } from '../context/AdminContext'; // ✅ Contextdan logout olish
...
const AdminLayout = () => {
  ...
  const { logout } = useAdmin(); // ✅ Contextdan logout

  // ✅ Admin sahifa faolligini boshqarish
  useEffect(() => {
    localStorage.setItem('adminActive', 'true'); // Kirganda
    return () => {
      localStorage.removeItem('adminActive'); // Chiqganda
    };
  }, []);

  // ✅ Logout handler (faqat admin key’lar tozalanadi)
  const handleLogout = () => {
  ...
  return (
    <div className="bg-[#F7F7F7] min-h-screen">
      ...
      <nav className="flex items-center justify-between py-4 px-6 sm:px-[4%] bg-white shadow-sm relative">
        <div>
          <img className="h-14 sm:h-20 object-contain" src={logo} alt="Complex Solutions Logo" />
        </div>
